...
async function ensureAccess(request, env, requiredRoles) {
  ...
  if (!auth.ok) {
    return { ok: false, response: json({ ok: false, error: auth.reason || 'unauthorized' }, auth.status || 401) };
  }

  const rateState = applyRateLimit({
    namespace: 'admin-orcamentos',
    key: auth.actor.id || auth.actor.email || getClientIp(request) || 'anonymous',
    limit: 90,
    windowMs: 60 * 1000,
    blockMs: 60 * 1000,
  });
...
async function listOrcamentos(request, env) {
  const access = await ensureAccess(request, env, ['viewer', 'operacao', 'comercial', 'admin']);
...
async function updateOrcamento(request, env) {
  const access = await ensureAccess(request, env, ['operacao', 'comercial', 'admin']);
  if (!access.ok) return access.response;
  ...
  if (!id) return json({ ok: false, error: 'id_invalido' }, 400);
  ...
  if (body?.pacote_sugerido !== undefined) payload.pacote_sugerido = stripDangerousText(String(body.pacote_sugerido || ''), 120);
  if (body?.status_orcamento !== undefined) payload.status = mapOrcStatusToDeal(String(body.status_orcamento || ''));
  if (body?.urgencia !== undefined) payload.urgencia = stripDangerousText(String(body.urgencia || ''), 20);
  if (body?.prioridade !== undefined) payload.prioridade = stripDangerousText(String(body.prioridade || ''), 20);
  if (body?.proxima_acao !== undefined) payload.proxima_acao = stripDangerousText(String(body.proxima_acao || ''), 300);
  if (body?.responsavel !== undefined) payload.responsavel = stripDangerousText(String(body.responsavel || ''), 120);
  ...
  if (body?.observacoes_internas !== undefined) payload.observacoes_internas = stripDangerousText(String(body.observacoes_internas || ''), 2000);
  if (body?.link_pdf !== undefined) payload.link_pdf = stripDangerousText(String(body.link_pdf || ''), 1000);
  ...
  await logAdminRequestEvent(env, request, {
    ...
    status: 'ok',
    origin: 'admin-orcamentos',
    actorUserId: access.auth.actor.id,
    actorEmail: access.auth.actor.email,
    actorRole: access.auth.actor.role,
    resourceType: 'deal',
    resourceId: id,
